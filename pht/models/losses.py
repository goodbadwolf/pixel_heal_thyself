...
class GANLoss(nn.Module):
    ...
    def __init__(self, type="nsgan", target_real_label=1.0, target_fake_label=0.0):
        ...
        if type == "nsgan":
            ...
            raise NotImplementedError(f"GAN type {type} is not found!")
    ...
    def hinge_loss(self, input, target, is_discriminator):
        ...
        if is_discriminator:
            return (
                criterion(1 - input).mean() if target else criterion(1 + input).mean()
            )
        else:
            return (-input).mean()
...
class SSIMLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        scale = torch.maximum(
            target.max(dim=1, keepdim=True)[0], torch.tensor(1.0, device=target.device)
        )
