...
def train_SANet(
    ...
    print("\t-Creating MambaNet")
    padding_mode = "replicate" if deterministic else "reflect"
    print("\t\t-MambaNet padding mode: %s" % padding_mode)
    print("\t\t-MambaNet curve order: %s" % cfg.trainer.curve_order)
    print("\t\t-MambaNet L1 lossW: %s" % cfg.trainer.l1_loss_w)
    print("\t\t-MambaNet GAN lossW: %s" % cfg.trainer.gan_loss_w)
    print("\t\t-MambaNet GP lossW: %s" % cfg.trainer.gp_loss_w)
    if cfg.trainer.use_lpips_loss:
        print("\t\t-MambaNet LPIPS lossW: %s" % cfg.trainer.lpips_loss_w)
    if cfg.trainer.use_ssim_loss:
        print("\t\t-MambaNet SSIM lossW: %s" % cfg.trainer.ssim_loss_w)
    if cfg.trainer.use_multiscale_discriminator:
        print("\t\t-MambaNet multiscale discriminator")
    if cfg.trainer.use_film:
        print("\t\t-MambaNet use FiLM")
    pos_encoder = PositionalEncoding2D(
        cfg.model.base_ch, cfg.data.patches.patch_size, cfg.data.patches.patch_size
    ).to(device)
    G = MambaDenoiserNet(
        cfg.model.in_ch,
        cfg.model.aux_in_ch,
        ...
        num_gcp=cfg.model.num_gcp,
    ).to(device)
    if cfg.trainer.use_multiscale_discriminator:
        D = MultiScaleDiscriminator(
            in_nc=cfg.model.in_ch, patch_size=cfg.data.patches.patch_size
        ).to(device)
    else:
        D = DiscriminatorVGG(3, 64, cfg.data.patches.patch_size).to(device)
    if cfg.trainer.get("load_model", False):
        G.load_state_dict(
            torch.load(os.path.join(cfg.trainer.get("model_path", None), "G.pt"))
        )
        D.load_state_dict(
            torch.load(os.path.join(cfg.trainer.get("model_path", None), "D.pt"))
        )
    ...
    lpips_loss = (
        lpips.LPIPS(net="vgg").to(device) if cfg.trainer.use_lpips_loss else None
    )
    ssim_loss = (
        SSIMLoss(window_size=11).to(device) if cfg.trainer.use_ssim_loss else None
    )

    milestones = [
        i * cfg.trainer.lr_milestone - 1
        for i in range(1, cfg.trainer.epochs // cfg.trainer.lr_milestone)
    ]
    optimizer_generator = optim.Adam(
        G.parameters(), lr=cfg.trainer.lrG, betas=(0.9, 0.999), eps=1e-8
    )
    scheduler_generator = lr_scheduler.MultiStepLR(
        optimizer_generator, milestones=milestones, gamma=0.5
    )
    optimizer_discriminator = optim.Adam(
        D.parameters(), lr=cfg.trainer.lrD, betas=(0.9, 0.999), eps=1e-8
    )
    ...
    for epoch in range(cfg.trainer.epochs):
        ...
        for i_batch, batch_sample in enumerate(train_dataloader):
            ...
            )  # normal is not yet preprocessed
            ...
            if cfg.trainer.use_multiscale_discriminator:
                ...
                ) / 2 + cfg.trainer.gp_loss_w * loss_gp
            discriminator_loss.backward()
            optimizer_discriminator.step()
            accumulated_discriminator_loss += (
                discriminator_loss.item() / cfg.trainer.batch_size
            )
            ...
            try:
                ...
                break
            generator_loss = (
                cfg.trainer.gan_loss_w * loss_g_fake + cfg.trainer.l1_loss_w * loss_l1
            )
            ...
            print(
                "\r\t-Epoch: %d \tTook: %f sec \tIteration: %d/%d \tIter Took: %f sec \tI/O Took: %f sec \tG Loss: %f \tD Loss: %f"
                % (
                    epoch + 1,
                    end - start,
                    i_batch + 1,
        ...
        if epoch % cfg.trainer.save_interval == 0:
            current_save_path = create_folder(
                os.path.join(root_save_path, "model_epoch%d" % (epoch + 1))
            )
            avg_psnr = 0.0
            avg_ssim = 0.0
            avg_mrse = 0.0
            ...
            with torch.no_grad():
                ...
                for i_batch, batch_sample in enumerate(val_dataloader):
                    ...
                    print(
                        "\r\t-Validation: %d \tTook: %f seconds \tIteration: %d/%d"
                        % (epoch + 1, end - start, i_batch + 1, val_num_samples),
                        end="",
                        flush=True,
                    )
                G.train()
                ...
                print(
                    "\r\t-Validation: %d \tTook: %d seconds \tAvg MRSE: %f \tAvg PSNR: %f \tAvg 1-SSIM: %f"
                ...
                with open(os.path.join(root_save_path, "evaluation.txt"), "a") as f:
                    f.write(
                        "Validation: %d \tAvg MRSE: %f \tAvg PSNR: %f \tAvg 1-SSIM: %f\n"
...
def train(cfg: DictConfig):
    ...
    if cfg.trainer.get("deterministic", False):
        ...
        val_dataloader = DataLoaderX(
            val_dataset, batch_size=1, shuffle=False, num_workers=7, pin_memory=True
        )
