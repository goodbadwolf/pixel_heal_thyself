...
class ConfigRegistry:
    ...
    @classmethod
    def register_model_config(cls, name: str, config_class: Type[BaseModelConfig]) -> None:
    ...
    def validate_config(cls, config: Config) -> bool:
        ...
        if not isinstance(config.model, model_class):
            raise TypeError(f"Expected model config of type {model_class.__name__}, got {type(config.model).__name__}")
